...
export interface VideoDetailProps {
  ...
  onUpdate: (data: Partial<VideoDetailProps['data']>) => void;
  onClose?: () => void;
}

export function VideoDetail({ data, onUpdate, onClose }: VideoDetailProps) {
  const [activeTab, setActiveTab] = useState<'basic' | 'animation'>('basic');

  const handleChange = (key: keyof VideoDetailProps['data'], value: any) => {
    onUpdate({ [key]: value });
  };
    ...
    >
      {activeTab === 'basic' ? (
        <>
          ...
          <SectionTitle className="mt-4">音量</SectionTitle>
          <FormItem label="音量">
            <div className="flex gap-3 items-center">
              <Slider
                value={[data.volume]}
          ...
          {data.opacity !== undefined && (
            <>
              <SectionTitle className="mt-4">其他</SectionTitle>
              <FormItem label="不透明度">
                <div className="flex gap-3 items-center">
                  <Slider
