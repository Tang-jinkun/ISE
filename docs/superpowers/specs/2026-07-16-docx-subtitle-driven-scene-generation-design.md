...
- 当前印巴场景必须登记全部 21 条现有航迹，并从目录中为批准蓝图要求的每个飞机和导弹 ActorInstance 分配唯一航迹，不得为主要对象重新合成路径；未被当前蓝图选中的航迹保留为目录容量，不得为了“用完资产”反向增加演员数量；
...
- 文件名中的平台或呼号只作为目录标签，不自动成为事实。当前人工工程的字幕明确把 `Vampire-2` 称为苏-30MKI，因此当前 scenario mapping 可将 ADAMPUR Vampire 束作为苏-30MKI 呼号束；J-10CE 航迹标签与文档 JF-17 演员的对应只在当前人工工程映射范围内成立，并保留命名差异诊断；
...
- 同一连续运动段中的 ActorInstance 始终保持同一 `trajectoryAssetRef`；只有行为明确进入新的连续运动段时才允许切换航迹，并记录切换 lineage；
...
- 当前 scenario mapping 依据用户提供的人工工程，将 4 条 MINAS 和 4 条 RAFIKI J-10CE 标签航迹绑定为文档 JF-17 演员的可选路径，并记录 `OPERATOR_ROUTE_LABEL_DIFFERS_FROM_REPORT_ENTITY`；
- 当前 scenario mapping 依据人工工程字幕，将 ADAMPUR Vampire 解释为苏-30MKI 呼号束，而不是平台型号；
- 这两项映射只对当前印巴场景有效，不进入全局实体同义词表；
- 当前 blueprint 是否使用 AMBALA Su-30MKI 的 2 条额外目录航迹由文档数量、分组和行为决定，不能为了消耗目录容量增加演员；
- 其他场景映射缺失时蓝图显示 warning 并等待确认，不能由模型或文件名猜测后静默成功。
...
- 经纬度、高度合法，时间严格单调；
- 速度、加速度和转弯率处于领域范围；
- 当前批准蓝图中的所有飞机和导弹 ActorInstance 均获得唯一现有航迹，且分配数量严格等于证据、用户值或默认策略决定的数量；
...
- 当前批准蓝图中的每个飞机和导弹对象获得唯一 ActorRouteAssignment，未选目录项不生成额外演员；
...
6. 当前印巴场景登记全部 18 条飞机和 3 条导弹现有航迹，并按批准蓝图数量进行唯一分配，未用航迹不反向增加演员，整个编译不调用 TrajectorySynthesizer；未来文档只有在目录、附件和文档坐标都不足时，才可经审核生成确定性示意航迹；
