...
import {
  ...
  type SqlJsPersistenceOptions,
} from './sqlJsDatabase.ts'
...
export class AgentDatabase implements SqliteDatabaseAdapter {
  ...
  static async open(
    path: string,
    driver: AgentSqliteDriver,
    persistenceOptions: SqlJsPersistenceOptions = {},
  ): Promise<AgentDatabase> {
    if (driver !== 'sql.js') {
      throw agentError(500, 'SQLITE_DRIVER_UNAVAILABLE', 'better-sqlite3 did not pass the runtime gate')
    }
    const adapter = await SqlJsDatabaseAdapter.open(path, persistenceOptions)
